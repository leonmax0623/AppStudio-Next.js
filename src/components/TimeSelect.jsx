  ...
  handlerShow(isShow = !this.state.isShow) {
    this.setState((state) => {
      ...
      if (value !== null && !this.isInitScroll) {
        ...
        const currentElem = this.parent.current.querySelector(`.timeSelect__dropItem[data-key="${findKey}"]`);
        
        this.parent.current.querySelector('.timeSelect__drop').scrollTop = currentElem.offsetTop;
      }
    });
  }
    ...
    return (
      ...
      >
        ...
        <div className={`timeSelect__drop`}>
          {this.dropList &&
            this.dropList.filter(item=>item.key !== value).map((item, key) => (
              <div
